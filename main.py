...
def html_parser(content):
    ...
    if items_table:
        ...
        items_table = soup.find(
            'table',
            {'style': lambda s: s and 'font-size:13px;line-height:15px;' in s},
        )
    if items_table:
    ...
    for row in items_table.find_all('tr')[1:table_length]:
        ...
        if len(col) == 4 or len(col) == 5:
            ...
            if item_name != '':
                try:
                    item_weight = re.findall(
                        r'\d*\.\d+|\d+',
                        col[1].contents[0].replace(' ', ''),
                    )[0]
                except IndexError:
                    item_weight = 0
                try:
...
def mailbox_parser(mbox_filename):
    ...
    for message in mbox:
        ...
        if not message.is_multipart():
            raise 'Message is not multipart message'
        contents_text = []
        contents_html = []
            ...
            if subtype == 'html':
                ...
                if enc[0] is not None:
                    contents_html.append(
                        part.get_payload(decode=True).decode(enc[0])
                    )
                else:
